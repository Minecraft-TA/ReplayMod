...
public class MCVer {
    ...
    //#if MC>=11400
    //$$ public static Optional<Widget> findButton(Iterable<? extends IGuiEventListener> buttonList, @SuppressWarnings("unused") String text, @SuppressWarnings("unused") int id) {
        //#if MC>=11600
        //$$ final Text message = new TranslatableText(text);
        //#else
        //$$ final String message = I18n.format(text);
        //#endif
    //$$     for (IGuiEventListener e : buttonList) {
    //$$         if (e instanceof INestedGuiEventHandler) {
    //$$             Optional<Widget> button = findButton(((INestedGuiEventHandler) e).children(), text, id);
    //$$             if (button.isPresent()) {
    //$$                 return button;
    //$$             }
    //$$         }
    ...
    public static Vec3d getTrackedPosition(Entity entity) {
        return new Vec3d(entity.serverPosX / 4096.0, entity.serverPosY / 4096.0, entity.serverPosZ / 4096.0);
    }
    //#endif
