...
public class GuiHandler extends EventRegistrations {
    ...
    private void injectIntoIngameMenu(GuiScreen guiScreen, Collection<GuiButton> buttonList) {
    ...
    private void moveAllButtonsInRect(
        ...
        buttons.stream()
                .filter(button -> button.x <= xEnd && button.x + button.width >= xStart)
                .filter(button -> button.y <= yEnd && button.y + button.height >= yStart)
    ...
    private void ensureReplayStopped(GuiScreen guiScreen) {
    ...
    private void injectIntoMainMenu(GuiScreen guiScreen, Collection<GuiButton> buttonList) {
    ...
    private void legacyInjectIntoMainMenu(GuiScreen guiScreen, Collection<GuiButton> buttonList) {
    ...
    private Point determineButtonPos(MainMenuButtonPosition buttonPosition, GuiScreen guiScreen, Collection<GuiButton> buttonList) {
        ...
        if (buttonPosition == MainMenuButtonPosition.TOP_LEFT) {
            ...
            return Stream.of(
                    ...
                    .filter(it -> buttonList.stream().noneMatch(button ->
                            button.x <= it.x + it.width + 4 + 20
                                    && button.y <= it.y + it.height
                                    && button.x + button.width >= it.x + it.width + 4
                                    && button.y + button.height >= it.y
                    ))
                    // then take the bottom-most and if there's two, the right-most
                    .max(Comparator.<GuiButton>comparingInt(it -> it.y).thenComparingInt(it -> it.x))
                    // and place ourselves next to it
                    .map(it -> new Point(it.x + it.width + 4, it.y))
            ...
            return Optional.of(buttonList).flatMap(buttons -> {
                ...
                switch (buttonPosition) {
                    ...
                    case RIGHT_OF_MULTIPLAYER:
                    case RIGHT_OF_REALMS:
                        return new Point(button.x + button.width + 4, button.y);
                }
                throw new RuntimeException();
            }).orElse(topRight);
        }
    ...
    private int determineButtonIndex(Collection<GuiButton> buttons, GuiButton button) {
        GuiButton best = null;
        int bestIndex = -1;
